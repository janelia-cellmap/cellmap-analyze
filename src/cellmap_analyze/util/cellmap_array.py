...
class CellMapArray:
    ...
    Provides ROI-based indexing that converts between physical coordinates
    and voxel indices, replacing funlib.persistence.Array.
    """

    def __init__(self, data, voxel_size, offset):
        """
        Args:
            data: A zarr.Array instance.
            voxel_size: Coordinate or tuple of voxel dimensions in physical units.
            offset: Coordinate or tuple of the array origin in physical units.
        """
        self.data = data
        self.voxel_size = (
    ...
    def __setitem__(self, roi, value):
        if isinstance(roi, Roi):
            self.data[self._to_slices(roi)] = value
        else:
            self.data[roi] = value

    def __getitem__(self, roi):
        if isinstance(roi, Roi):
            return np.array(self.data[self._to_slices(roi)])
        return np.array(self.data[roi])
